...
// WorkingDirectory= does not strip quotes (only Exec* directives do), so it
// must be unquoted. Specifiers still expand, hence the %% escaping.
...
export function serviceUnit(
  ...
  if (/\s/.test(root))
    throw new Error(
      "The checkout path contains whitespace, which systemd WorkingDirectory= does not support. Move the checkout to a path without spaces.",
    );
  return `# Generated by 9t setup --service. Safe to replace by re-running it.\n[Unit]\nDescription=9t personal workspace\nAfter=network.target\n\n[Service]\nType=simple\nUser=${username}\nWorkingDirectory=${unitPath(root)}\nEnvironment=NODE_ENV=production\nExecStart=${unitQuote(executable)} ${unitQuote(join(root, "scripts/run-server.mjs"))}\nRestart=on-failure\nRestartSec=3\nNoNewPrivileges=true\nPrivateTmp=true\nUMask=0077\n\n[Install]\nWantedBy=multi-user.target\n`;
}
...
export async function installService(root, port) {
  ...
  if (existsSync(unitPath)) {
    let previous = "";
    try {
      previous = readFileSync(unitPath, "utf8");
    } catch {}
    if (!previous.includes("Generated by 9t setup"))
      throw new Error(
        `A service file already exists at ${unitPath} and was not created by 9t. It will not be replaced.`,
      );
  }
  ...
  await privileged("install", ["-m", "644", file, unitPath]);
  ...
  try {
    ...
    throw new Error(
      `Service ${name} is installed but not running. Inspect it with: systemctl status ${name} — journalctl -u ${name} -f`,
    );
  }
  let healthy = false;
