...
final class LocalStore extends SQLiteOpenHelper {
  ...
  List<JSONObject> items(String where) throws Exception {
    ...
    try (Cursor c =
        getReadableDatabase().query("inbox", null, where, null, null, null, "revision DESC")) {
  ...
  List<JSONObject> outbox() throws Exception {
    List<JSONObject> list = new ArrayList<>();
    try (Cursor c = getReadableDatabase().rawQuery("SELECT id,content,error FROM outbox", null)) {
      while (c.moveToNext())
        list.add(
            new JSONObject()
